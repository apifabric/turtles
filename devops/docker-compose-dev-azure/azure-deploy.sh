...
resourcename="turtles"  # lower case, only
resourcegroup="turtles_rg"
...
# git clone https://github.com/apilogicserver/turtles.git
# cd turtles
...
az group create --name turtles_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group turtles_rg --sku S1 --is-linux
az container create --resource-group turtles_rg --name turtles --image apilogicserver/turtles:latest --dns-name-label turtles --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//turtles.westus.azurecontainer.io:5656'
